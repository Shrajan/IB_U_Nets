...
class IBConvLayers(nn.Module):
    def __init__(self, input_feature_channels, output_feature_channels, num_convs,
                 ...
                 dropout_op=nn.Dropout2d, dropout_op_kwargs=None,
                 nonlin=nn.LeakyReLU, nonlin_kwargs=None, first_stride=None, basic_block=ConvDropoutNormNonlin,
                 ib_strides=(2,2,2), ib_k=5, ib_input_features=1):
        '''
        stacks ConvDropoutNormLReLU layers. initial_stride will only be applied to first layer in the stack. The other parameters affect all layers
        :param input_feature_channels:
        :param output_feature_channels:
        ...
        if conv_op == nn.Conv2d:
            ...
            if ib_k == 3:
                radius = 1.0
                gamma = 1.0/2.0
                self.ib_paddings = (1,1,1)
            elif ib_k == 5:
                radius = 2.0
                gamma = 2.0/3.0
                self.ib_paddings = (2,2,2)
            elif ib_k == 7:
                radius = 3.0
                gamma = 3.0/4.0
                self.ib_paddings = (3,3,3)
            elif ib_k == 9:
                radius = 4.0
                gamma = 4.0/5.0
                self.ib_paddings = (4,4,4)
            else:
                raise ValueError("unknown IB kernel size: %s" % str(ib_k))
    ...
    def forward(self, ib_conv_in, encoder_in):
        output1 = self.conv1(encoder_in)
        output2 = self.conv2(encoder_in)

        # On and Off surround modulation
        sm_on = self.lrelu(self.surround_modulation_DoG_on(ib_conv_in))
        sm_off = self.lrelu(self.surround_modulation_DoG_off(ib_conv_in))
    ...
    def surround_modulation_DoG_on(self, input):
        if self.conv_op == nn.Conv2d:
            return F.conv2d(input, weight=self.conv_On_filters, stride=tuple(self.ib_strides), padding=self.ib_paddings)
        else:
            return F.conv3d(input, weight=self.conv_On_filters, stride=tuple(self.ib_strides), padding=self.ib_paddings)

    def surround_modulation_DoG_off(self, input):
        if self.conv_op == nn.Conv2d:
            return F.conv2d(input, weight=self.conv_Off_filters, stride=tuple(self.ib_strides), padding=self.ib_paddings)    
        else:
            return F.conv3d(input, weight=self.conv_Off_filters, stride=tuple(self.ib_strides), padding=self.ib_paddings)
...
# IB kernels are applied to the output of the first enconder.
class IB_UNet_3D(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        """
        super(IB_UNet_3D, self).__init__()
        ...
        self.ib_kernel_size = ib_kernel_size
        ...
        for d in range(num_pool):
            ...
            if d == 1:
                self.conv_blocks_context.append(IBConvLayers(input_features, output_features, num_conv_per_stage,
                                                                ...
                                                                ib_k=self.ib_kernel_size,
                                                                ib_input_features=input_features))
            else:
...
# IB kernel are applied to the network input.
class IB_UNet_2D(SegmentationNetwork):
    DEFAULT_BATCH_SIZE_3D = 2
    DEFAULT_PATCH_SIZE_3D = (64, 192, 160)
    SPACING_FACTOR_BETWEEN_STAGES = 2
    BASE_NUM_FEATURES_3D = 30
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
        ...
        Questions? -> f.isensee@dkfz.de
        """
        super(IB_UNet_2D, self).__init__()
        self.convolutional_upsampling = convolutional_upsampling
        self.convolutional_pooling = convolutional_pooling
        self.upscale_logits = upscale_logits
        if nonlin_kwargs is None:
        ...
        for d in range(num_pool):
            ...
            if d == 1:
                self.conv_blocks_context.append(IBConvLayers(input_features, output_features, num_conv_per_stage,
                                                                ...
                                                                ib_strides=pool_op_kernel_sizes[d - 1],
                                                                ib_k=self.ib_kernel_size,
                                                                ib_input_features=input_channels))
            else:
                self.conv_blocks_context.append(StackedConvLayers(input_features, output_features, num_conv_per_stage,
                                                                self.conv_op, self.conv_kwargs, self.norm_op,
                                                                self.norm_op_kwargs, self.dropout_op,
    ...
    def forward(self, x):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            ...
            if d != 1:
                ...
                x = self.conv_blocks_context[d](network_in, x)
                skips.append(x)
                if not self.convolutional_pooling:
                    x = self.td[d](x)
...
# Latest version - In both the 2D and 3D versions, the IB convolution is performed on the output of the first encoder block and not directly on the input.
